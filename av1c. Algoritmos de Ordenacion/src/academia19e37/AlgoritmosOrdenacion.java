...
public class AlgoritmosOrdenacion {
	...
	static void quickSortRecursive(Integer l[], int izquierda, int derecha) {
		int pivote = l[izquierda]; // Cogemos el primero como pivote (optimizable)
		int i = izquierda; // i recorrerá la lista de izquierda a derecha.
		int j = derecha; // i recorrerá la lista de derecha a izquierda.
		...
		if (izquierda < j - 1)
			quickSortRecursive(l, izquierda, j - 1); // Ordenamos la subcadena
													// izquierda
		if (j + 1 < derecha)
			quickSortRecursive(l, j + 1, derecha); // Ordenamos la subcadena
														// derecha
	}
